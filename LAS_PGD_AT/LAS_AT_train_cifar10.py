...
eps = np.finfo(np.float32).eps.item()  # 替代可能计算中可能出现的零
...
def Get_reward(input_batch, y_batch, target_model, action):
    ...
    criterion = nn.CrossEntropyLoss()
    inputs, targets = input_batch.cuda(), y_batch.cuda()
    delta = Attack_policy_batch(input_batch, y_batch, target_model, action)   #### 计算出扰动数值
    with torch.no_grad():
    ...
    copy_target_model = train_target_model(inputs + delta, targets, copy_target_model) #### 利用一阶梯度下降更新模型参数
    epsilon = (8 / 255.) / std
    alpha = (2 / 255.) / std
    pgd_delta = attack_pgd(copy_target_model, inputs, targets, epsilon, alpha, 10, 2) #### 论文中的aj
...
def test(epoch):
    ...
    if epoch % 20 == 0:
        policy_path_epoch = os.path.join(out_dir, f'{epoch}_policy_model_ckpt.t7')  # 每二十轮把StrategyNet保存下来
        torch.save(state1, policy_path_epoch)
